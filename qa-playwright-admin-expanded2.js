...
async function main() {
  ...
  // Buscar el botón de expansión por su clase o icono
  const expandBtn = page.locator('button:has(.pi-chevron-down), button:has(.pi-angle-down), [data-p-icon="pi-chevron-down"]').first();
  if (await expandBtn.count()) {
    await expandBtn.click();
    await page.waitForTimeout(3000);
    await page.screenshot({ path: path.join(OUT_DIR, '23-admin-usuario-expanded-marcas.png'), fullPage: true });
  } else {
    // Buscar por el icono de chevron directamente
    ...
    if (await chevron.count()) {
      ...
      console.log('No se encontró chevron');
    }
  }
