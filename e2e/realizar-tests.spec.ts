...
test.describe('Realizar Tests - Configuración y Generación', () => {
  ...
  test.describe('Interfaz de configuración', () => {
    test('debería mostrar los controles de configuración', async ({ page }) => {
      await expect(
        page.locator('[data-testid="configuracion-test-card"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="num-preguntas-dropdown"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="dificultad-multiselect"]'),
      ).toBeVisible();
      await expect(
    ...
    test('el botón de generar está deshabilitado hasta que se seleccionen temas', async ({
      page,
    }) => {
      await expect(
        page.locator('[data-testid="generar-test-btn"]'),
  ...
  test.describe('Test Normal', () => {
    test('debería configurar y generar un test básico', async ({ page }) => {
      ...
      // Confirm dialog
      await expect(page.locator('.p-confirm-dialog').first()).toBeVisible({
        timeout: 5_000,
      });
  ...
  test.describe('Test de Examen (con cronómetro)', () => {
    test('debería mostrar campo de tiempo cuando se activa modo examen', async ({
      page,
    }) => {
      await page.locator('[data-testid="test-examen-switch"]').click();
      await expect(
        page.locator('[data-testid="tiempo-limite-input"]'),
      ).toBeVisible();
    });

    test('el botón queda deshabilitado si se activa examen sin tiempo', async ({
      page,
    }) => {
  ...
  test.describe('Tests Pendientes', () => {
    test('no muestra sección de pendientes cuando no hay tests en curso', async ({
      page,
    }) => {
      // setupTestGenerarInterceptors devuelve GET /tests → [] (array vacío); el
      // contenedor existe pero sin items de test pendiente.
      await expect(page.locator('[data-testid="test-pendiente"]')).toHaveCount(
        0,
      );
    });
    ...
    test('muestra tests pendientes cuando los hay', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="tests-pendientes-container"]'),
      ).toBeVisible({ timeout: 5_000 });
      await expect(
        page.locator('[data-testid="test-pendiente"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="continuar-test-btn"]'),
      ).toBeVisible();
    });

    test('abortar un test pendiente lo elimina de la lista', async ({
      page,
    }) => {
      let aborted = false;
      await page.route(/\/tests(\?.*)?$/, (route) => {
  ...
  test.describe('Ejecución de preguntas', () => {
    ...
    test('muestra la interfaz de pregunta correctamente', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="numero-pregunta"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="enunciado-pregunta"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="opcion-respuesta"]'),
      ).toHaveCount(4);
    });
